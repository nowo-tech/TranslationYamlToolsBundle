...
- **SecurityBundle** (required for the Web UI): form login at `/login`, firewall + `access_control` for `/_demo/translation-yaml-tools`, bundle `security.access_roles: [ROLE_ADMIN]`. Demo credentials: **`admin` / `admin`** (in-memory, plaintext hasher — local demos only). No `allow_unauthenticated`.
