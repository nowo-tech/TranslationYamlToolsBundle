...
| `missing_translation_log.enabled` | `bool` | `false` | When **`true`**, decorates **`translator`**, records missing keys per request, flushes on **`kernel.terminate`** (priority **`-1024`**). Persistence errors are logged and not rethrown (safe for FrankenPHP worker mode without `kernel.reset` — see [FRANKENPHP-WORKER-AUDIT](FRANKENPHP-WORKER-AUDIT.md)). In the usual **`index.php`** flow, **`terminate`** runs **after** **`$response->send()`**, so the client has already received the response body; the process may still run DB or Messenger work during terminate. Requires **`doctrine/orm`** and **`doctrine/doctrine-bundle`**; the bundle **prepends** ORM mapping for **`MissingTranslationLog`**. |
