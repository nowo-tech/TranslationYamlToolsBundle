...
final class TwigPathsPassTest extends TestCase
{
    ...
    public function testProcessReturnsEarlyWhenWebUiParameterMissing(): void
    {
        $container = new ContainerBuilder();
        $container->setParameter('kernel.project_dir', sys_get_temp_dir());
        $loaderDef = new Definition();
        ...
        self::assertSame([], $loaderDef->getMethodCalls());
    }

    public function testProcessUsesNativeLoaderDefinitionWhenRegisteredWithoutAlias(): void
    {
        $tmp       = sys_get_temp_dir() . '/tyt_twig_native_' . bin2hex(random_bytes(4));
        $loaderDef = new Definition();
        $container = new ContainerBuilder();
        $container->setParameter('kernel.project_dir', $tmp);
        $container->setParameter('nowo_translation_yaml_tools.missing_translation_log.web_ui.enabled', true);
    ...
    {
        $tmp = sys_get_temp_dir() . '/tyt_twig_chain_' . bin2hex(random_bytes(4));
        self::assertTrue(mkdir($tmp, 0777, true));
